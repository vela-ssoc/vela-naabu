...
import (
	...
	"github.com/vela-ssoc/vela-naabu/naabu/privileges"
			...
			for ip := range ipStream {
				...
				go func(ip string) {
					...
					for _, p := range data.Ports {
						r.scanner.ScanResults.AddPort(ip, &port.Port{Port: p, Protocol: protocol.TCP}, r.options.Callback)
					}
				}(ip)
			}
		}
...
func (r *Runner) handleHostPort(host string, p *port.Port) {
	...
	if open && err == nil {
		r.scanner.ScanResults.AddPort(host, p, r.options.Callback)
	}
}
